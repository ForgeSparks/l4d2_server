...
$totalpoints = $row['points'] + $row['points_survival'] + $row['points_survivors'] + $row['points_infected'] + $row['points_realism'] + $row['points_scavenge_survivors'] + $row['points_scavenge_infected'] + $row['points_realism_survivors'] + $row['points_realism_infected'] + $row['points_mutations'];
$rankrow = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS rank FROM players WHERE points + points_survival + points_survivors + points_infected + points_realism + points_scavenge_survivors + points_scavenge_infected + points_realism_survivors + points_realism_infected + points_mutations >= '" . $totalpoints . "'"));
$rank = $rankrow['rank'];
...
if (mysql_num_rows($result) > 0) {
  ...
  if (function_exists('bcadd'))
    $stats->set("player_url", "<a href=\"http://steamcommunity.com/profiles/" . getfriendid($row['steamid']) . "\">" . $playername . "'s Community Page</a>");
  else
    $stats->set("player_url", "<b>Community page URL disabled</b>");

  $stats->set("player_lastonline", date("M d, Y g:ia", $row['lastontime'] + ($dbtimemod * 3600)) . " (" . formatage(time() - $row['lastontime'] + ($dbtimemod * 3600)) . " ago)");
  ...
  if ($rank == 1) {
    ...
    $text_rank = "<div class='rank lvl10-999'>" . $rank . "</div>";
  }
  ...
  if ($row['infected_spawn_1'] == 0 || $row['infected_smoker_damage'] == 0)
    $stats->set("player_avg_smoker", "0");
  else
    $stats->set("player_avg_smoker", number_format($row['infected_smoker_damage'] / $row['infected_spawn_1'], 2));

  if ($row['infected_boomer_vomits'] == 0 || $row['infected_boomer_blinded'] == 0)
    $stats->set("player_avg_boomer", "0");
  else
    $stats->set("player_avg_boomer", number_format($row['infected_boomer_blinded'] / $row['infected_boomer_vomits'], 2));

  if ($row['infected_hunter_pounce_counter'] == 0 || $row['infected_hunter_pounce_dmg'] == 0)
    $stats->set("player_avg_hunter", "0");
  else
    $stats->set("player_avg_hunter", number_format($row['infected_hunter_pounce_dmg'] / $row['infected_hunter_pounce_counter'], 2));

  if ($row['infected_spawn_8'] == 0 || $row['infected_tank_damage'] == 0)
    $stats->set("player_avg_tank", "0");
  else
    $stats->set("player_avg_tank", number_format($row['infected_tank_damage'] / $row['infected_spawn_8'], 2));
