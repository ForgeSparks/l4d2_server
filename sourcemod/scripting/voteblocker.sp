...
public OnPluginStart()
{
	...
	CreateConVar("vb_version", VERSION, "Version of the installed plugin.", FCVAR_NOTIFY);

	// Create the rest of the convars for the plugin
	g_hEnable				= CreateConVar("vb_enable",				"1",	"0 - Disable plugin, 1 - Enable plugin",																						FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hLog					= CreateConVar("vb_log",				"0",	"0 - Disable logging of blocked votes, 1 - Enable logging of blocked votes",													FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hRespectLevels		= CreateConVar("vb_respectlevels",		"1",	"0 - Disable comparing immunity levels, 1 - Enable comparing immunity levels",													FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hBlockCount			= CreateConVar("vb_blockcount",			"3",	"0 - Disable blocked vote limit for clients, n - Maximum number of blocked votes per client per map before they are kicked",	FCVAR_NOTIFY, true, 0.0, true, 5.0);
	g_hAdminImmunity		= CreateConVar("vb_adminimmunity",		"1",	"0 - Disable admin kick immunity, 1 - Enable admin kick immunity",																FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hTankImmunity			= CreateConVar("vb_tankimmunity",		"0",	"0 - Disable kick immunity for tanks, 1 - Enable kick immunity for tanks",														FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hSpectatorVote		= CreateConVar("vb_spectatorvote",		"b",	"0 - Allow this type of vote, x - Only clients that match one or more of these flags can call this vote",						FCVAR_NOTIFY, true, 0.0, true, 0.0);
	g_hKick					= CreateConVar("vb_kick",				"0",	"0 - Allow this type of vote, x - Only clients that match one or more of these flags can call this vote",						FCVAR_NOTIFY, true, 0.0, true, 0.0);
	g_hReturnToLobby		= CreateConVar("vb_returntolobby",		"b",	"0 - Allow this type of vote, x - Only clients that match one or more of these flags can call this vote",						FCVAR_NOTIFY, true, 0.0, true, 0.0);
	g_hChangeAlltalk		= CreateConVar("vb_changealltalk",		"b",	"0 - Allow this type of vote, x - Only clients that match one or more of these flags can call this vote",						FCVAR_NOTIFY, true, 0.0, true, 0.0);
	g_hRestartGame			= CreateConVar("vb_restartchapter",		"0",	"0 - Allow this type of vote, x - Only clients that match one or more of these flags can call this vote",						FCVAR_NOTIFY, true, 0.0, true, 0.0);
	g_hChangeMission		= CreateConVar("vb_changemission",		"0",	"0 - Allow this type of vote, x - Only clients that match one or more of these flags can call this vote",						FCVAR_NOTIFY, true, 0.0, true, 0.0);
	g_hChangeChapter		= CreateConVar("vb_changechapter",		"0",	"0 - Allow this type of vote, x - Only clients that match one or more of these flags can call this vote",						FCVAR_NOTIFY, true, 0.0, true, 0.0);
	g_hChangeDifficulty		= CreateConVar("vb_changedifficulty",	"0",	"0 - Allow this type of vote, x - Only clients that match one or more of these flags can call this vote",						FCVAR_NOTIFY, true, 0.0, true, 0.0);
...
public Action:Listener_CallVote(client, const String:command[], argc)
{
	if (GetConVarInt(g_hEnable) == 1)
	{
		...
		GetClientAuthId(client, AuthId_Steam2, g_sCallerAuth[client], sizeof(g_sCallerAuth[]));
